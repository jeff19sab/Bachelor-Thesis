...
class PandaPickAndPlaceEnv(RobotTaskEnv):
    ...
    def __init__(
        ...
        sim = PyBullet(render_mode=render_mode, renderer=renderer)
        robot_1 = Panda(sim, block_gripper=False, base_position=np.array([-0.6, 0.0, 0.0]), control_type=control_type)
        #robot_2 = Panda(sim, block_gripper=True, base_position=np.array([0.6, 0.0, 0.0]), control_type=control_type)
...
class PandaReachEnv(RobotTaskEnv):
    ...
    def __init__(
        ...
        sim = PyBullet(render_mode=render_mode, renderer=renderer)
        #orientation = sim.physics_client.getQuaternionFromEuler([0,0,np.pi])
        #orientation_2 = sim.physics_client.getQuaternionFromEuler([0,0,5.498])
        robot_1 = Panda(sim, block_gripper=False, base_position=np.array([-0.6, 0.2, 0.0]), control_type=control_type, robot_name='panda_1') # right according to human view
        robot_2 = Panda(sim, block_gripper=False, base_position=np.array([-0.6, -0.2, 0.0]), control_type=control_type, robot_name='panda_2') #left according to human view
        task = Reach(sim, reward_type=reward_type, get_ee_position=robot_1.get_ee_position)
        super().__init__(
            robot_1,
            robot_2,
            task,
