...
class PyBulletRobot(ABC):
    ...
    def _load_robot(self, file_name: str, base_position: np.ndarray, base_orientation = None) -> None:
        """Load the robot.
...
class RobotTaskEnv(gym.Env):
    ...
    def __init__(
        self,
        robot_1: PyBulletRobot,
        robot_2: PyBulletRobot,
        task: Task,
        ...
        assert robot_1.sim == task.sim and robot_2.sim == task.sim, "Robots and the task must belong to the same simulation."
        self.sim = robot_1.sim  # Assuming both robots are in the same simulation
        ...
        self.action_space = spaces.Box(-1.0, 1.0, shape=( self.robot_1.action_space.shape[0]+ self.robot_2.action_space.shape[0] ,), dtype=np.float32)
    ...
    def _get_obs(self) -> Dict[str, np.ndarray]:
        robot_obs_1 = self.robot_1.get_obs().astype(np.float32)  # robot state
        robot_obs_2 = self.robot_2.get_obs().astype(np.float32)  # robot state
        task_obs = self.task.get_obs().astype(np.float32)  # object position, velococity, etc...
        observation = np.concatenate([robot_obs_1, robot_obs_2, task_obs])
    ...
    def step(self, action: np.ndarray) -> Tuple[Dict[str, np.ndarray], float, bool, bool, Dict[str, Any]]:
        self.robot_1.set_action(action[:4])
        self.robot_2.set_action(action[4:])
        self.sim.step()
        observation = self._get_obs()
