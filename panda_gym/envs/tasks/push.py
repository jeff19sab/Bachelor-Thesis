...
class Push(Task):
    ...
    def _create_scene(self) -> None:
        ...
        self.sim.physics_client.setVRCameraState([1.5, -0.5, -1.0])
        self.sim.physics_client.loadURDF("cube_small.urdf", [0.03843895, 0.1, 0])
        ...
        for jointIndex2 in range(self.sim.physics_client.getNumJoints(self.pr2_gripper2)):
            ...
            self.sim.physics_client.setJointMotorControl2(self.pr2_gripper2, jointIndex2,
                                                          ...
                                                          targetPosition=0,
                                                          force=0)
        self.pr2_cid = self.sim.physics_client.createConstraint(self.pr2_gripper, -1, -1, -1,
        ...
        self.pr2_cid4 = self.sim.physics_client.createConstraint(self.pr2_gripper2,
                                                                 ...
                                                                 parentFramePosition=[0, 0, 0],
                                                                 childFramePosition=[0, 0, 0])
        self.sim.physics_client.changeConstraint(self.pr2_cid2, gearRatio=1, erp=0.5, relativePositionTarget=0.5,
                                                 maxForce=3)
        self.sim.physics_client.changeConstraint(self.pr2_cid4, gearRatio=1, erp=0.5, relativePositionTarget=0.5,
                                                 maxForce=3)
