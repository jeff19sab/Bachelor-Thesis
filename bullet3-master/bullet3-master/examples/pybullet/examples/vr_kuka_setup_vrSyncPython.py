...
def get_handler_position(controller_id):
    handler_position = [0, 0, 0]  # Default position

    events = p.getVREvents()
    for e in events:
        if e[CONTROLLER_ID] == controller_id:
            handler_position = e[POSITION]
            break
...
while (1):
    ...
    # Call get_handler_position to get the controller position
    controller_position = get_handler_position(controllerId)
    print(controller_position)
    # Check if the position is non-zero before printing
    # if controller_position != [0, 0, 0]:
    #     print("Controller position:", controller_position)
