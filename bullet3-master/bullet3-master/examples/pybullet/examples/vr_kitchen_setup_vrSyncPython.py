...
def get_controller_positions():
    """
    Retrieve the 3D positions of the VR controllers and add them to a list.

    Returns:
        List of 3D positions of VR controllers.
    """
    controller_positions = []
    events = p.getVREvents()
    for e in events:
        if e[CONTROLLER_ID] != -1:  # Ensure it's a valid controller event
            controller_positions.append(e[POSITION])
    return controller_positions

controller_positions = get_controller_positions()
print("Controller Positions:", controller_positions)
